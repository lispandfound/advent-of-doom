...
-- Given a map here the keys are co-ordinates, returns the minimum x, maximum x, minimum y, and maximum y; in that order.
mapBoundingBox :: Map (Int, Int) a -> Maybe (Int, Int, Int, Int)
mapBoundingBox m =
  (,,,)
    <$> minimumOf (folded . _1) (Map.keys m)
    <*> maximumOf (folded . _1) (Map.keys m)
    <*> minimumOf (folded . _2) (Map.keys m)
    <*> maximumOf (folded . _2) (Map.keys m)
...
mapAdjacent f l = zipWith f l (drop 1 l)
...
foldr1 :: (b -> b -> b) -> NonEmpty b -> b
foldr1 f l = foldr f (head l) (tail l)

-- crtSystem solves the simultaneous equations x = xi mod mi for some list [(xi, mi), ...]
-- assumes gcd mi mj == 1
-- returns the smallest positive solution
crtSystem :: (Integral a) => NonEmpty (a, a) -> a
