...
module Program.RunDay (runDay, Day, Verbosity (Quiet, Timings, Verbose), runDayNoPrintInput) where
...
runDayNoPrintInput :: (Show a, Show b) => Parser i -> (i -> a) -> (i -> b) -> Program.RunDay.Day
runDayNoPrintInput inputParser partA partB verbosity inputFile = do
    input <- runExceptT $ do
        inputFileExists <- liftIO $ doesFileExist inputFile
        fileContents <-
            if inputFileExists
        ...
        case parseOnly inputParser . pack $ fileContents of
            ...
            Right i -> do
                when (verbosity == Verbose) $ do
                    liftIO $ putStrLn "Parser output unavailable."
                return i
